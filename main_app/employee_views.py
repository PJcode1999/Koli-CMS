...
def employee_home(request):
    ...
    clocks = Timesheet.objects.filter(custom_user=employee.admin,)
    todays_clocks =  clocks.filter(recorded_datetime=date.today())
    
    summary = clocks.aggregate(
        total_clocked_hours=Sum('clocked_hours'),
        total_break_hours=Sum('break_hours'),
        total_payroll_hours=Sum('total_payroll_hours'),
    )
    context = {
        'total_attendance': total_attendance,
        "summary":summary,
        "todays_clocks":todays_clocks,
...
def employee_apply_leave(request):
    ...
    context = {
        'form': form,
        'leave_history': LeaveReportEmployee.objects.filter(employee=employee,status__in=[1,2]).order_by("-created_at"),
